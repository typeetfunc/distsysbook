...
Синхронизация часто применяется "в лоб" для всех операций в системе, когда только порядок некотрого подмножества операций влияет на конечный результат.  Когда порядок необходим для обеспечения корректности? CALM теорема - о которой будет вестись речь в следующей главе - дает единственный верный ответ.
